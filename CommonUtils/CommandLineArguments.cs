    ...
    public sealed class Parser
    {
        ...
        public static bool ParseArgumentsWithUsage(string [] arguments, object destination)
        {
            if (Parser.ParseHelp(arguments) || !Parser.ParseArguments(arguments, destination))
            {
                ...
                Log.Error(Parser.ArgumentsUsage(destination.GetType()));
                return false;
            }
        ...
        public Parser(Type argumentSpecification, ErrorReporter reporter)
        {
            ...
            foreach (PropertyInfo prop in argumentSpecification.GetProperties())
            {
                if (prop.CanRead && prop.CanWrite)
                {
                    ...
                    {
                        Console.WriteLine("Skipping prop {0}", prop);
                        //this.arguments.Add(new Argument(attribute, prop, reporter));
                    }
                }
            }
        ...
        private static ArgumentAttribute GetAttribute(MemberInfo storage)
        {
            object[] attributes = storage.GetCustomAttributes(typeof(ArgumentAttribute), false);
        ...
        private static ArgumentAttribute GetAttribute(PropertyInfo prop)
        {
            object[] attributes = prop.GetCustomAttributes(typeof(ArgumentAttribute), false);
            if (attributes.Length == 1)
                return (ArgumentAttribute)attributes[0];
        ...
        private static string LongName(ArgumentAttribute attribute, MemberInfo storage)
        {
            return (attribute == null || attribute.DefaultLongName) ? storage.Name : attribute.LongName;

            throw new Exception("Unhandled type");
        }

        private static string ShortName(ArgumentAttribute attribute, MemberInfo storage)
        {
            if (attribute is DefaultArgumentAttribute)
                return null;
            if (!ExplicitShortName(attribute))
                return LongName(attribute, storage).Substring(0,1);
        ...
        private static object DefaultValue(ArgumentAttribute attribute)
        {
            ...
            private static Type MemberType(MemberInfo member)
            {
                ...
                //throw new Exception("Unhandled MemberInfo");
                return null;
            }
            
        private static Type ElementType(MemberInfo member)
        {
            Type type = MemberType(member);
            if (IsGenericList(type))
                return GetGenericListType(type);
        ...
        private static ArgumentType Flags(ArgumentAttribute attribute, MemberInfo member)
        {
            if (attribute != null)
                return attribute.Type;
            else if (IsCollectionType(member.GetType()))
        ...
        {
            return type.IsArray || IsGenericList(type);
        }
        ...
        private static bool IsGenericStringList(Type type)
        {
            return IsGenericList(type) && (GetGenericListType(type) == typeof(string));
        }
        ...
        private static bool IsValidElementType(Type type)
        {
            return type != null && (
                ...
                type == typeof(string) ||
                type == typeof(bool) ||
                IsGenericStringList(type) ||
                type.IsEnum);
        }
        
        private class Argument
        {
            public Argument(ArgumentAttribute attribute, MemberInfo storage, ErrorReporter reporter)
            {
                this.longName = Parser.LongName(attribute, storage);
                this.explicitShortName = Parser.ExplicitShortName(attribute);
                this.shortName = Parser.ShortName(attribute, storage);
                ...
                this.elementType = ElementType(storage);
                this.flags = Flags(attribute, storage);
                this.member = storage;
                ...
                Debug.Assert(!this.HasDefaultValue || (this.defaultValue.GetType() == storage.GetType()), "Type of default value must match field type");
            }

            private void SetMemberValue(MemberInfo member, object destination, object value)
            {
                if ((member as FieldInfo) != null)
                {
                    (member as FieldInfo).SetValue(destination, value);
                }
                else if ((member as PropertyInfo) != null)
                {
                    if (IsGenericStringList(MemberType(member)))
                    {
                        List<string> valueList = new List<string>();
                        valueList.AddRange((string[]) value);
                        value = valueList;
                    }

                    (member as PropertyInfo).SetValue(destination, value);
                }
                else
                {
                    throw new Exception("Unhandled MemberInfo");
            ...
            public bool Finish(object destination)
            {
                ...
                {
                    this.SetMemberValue(this.member, destination, this.DefaultValue);
                }
                if (this.IsCollection)
                {
                    this.SetMemberValue(this.member, destination, this.collectionValues.ToArray(this.elementType));
                }
            ...
            public bool SetValue(string value, object destination)
            {
                ...
                {
                    this.SetMemberValue(this.member, destination, newValue);
                }
                    ...
                    else
                    {
                        ...
                        else
                        {
                            ...
                            foreach (MemberInfo member in members)
                            {
                                ...
                                else if ((member is PropertyInfo) && (member as PropertyInfo).GetGetMethod().IsStatic)
                                {
                                    if (first)
                                        first = false;
                                    else
                                ...
                                {
                                    throw new Exception("Unhandled MemberInfo type");
                                }
                            }
                            builder.Append('}');
                        }
            ...
            public FieldInfo GetMemberAsField
            {
                get
                {
                    var realMember = this.member as FieldInfo;
                    return realMember;
                }
            }

            public PropertyInfo GetMemberAsProperty
            {
                get
                {
                    var realMember = this.member as PropertyInfo;
                    return realMember;
                }
            }
            ...
            public Type Type
            {
                get
                {
                    if (GetMemberAsField != null)
                        return GetMemberAsField.FieldType;
                    else if (GetMemberAsProperty != null)
                        return GetMemberAsProperty.PropertyType;
                    throw new Exception("Unhandled MemberInfo");
                }
            }
